...
class Vartotojas extends CI_Controller {
    ...
    public function rasytiAtsiliepima() {
        ...
        if($query=$this->CRUD_model->get_reviews()){
            ...
            $this->load->view("header");
            $this->load->view("rasytiAtsiliepima",$data);
            $this->load->view("footer");
        }
        else{
            redirect('index.php/vartotojas/atsiliepimai');
        }
    }
    ...
    public function adminasPrisijungti() {
        ...
        $this->load->library('form_validation');
        $this->form_validation->set_rules('vardas','Vardo','required|trim|xss_clean|callback_validate_creds');
        $this->form_validation->set_rules('slaptazodis','Slaptažodžio','required|md5|trim');
